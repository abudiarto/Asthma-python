...
def lateFussion(vocab_size, month_size, max_codes, lr, clipvalue):
...
def earlyFussion(vocab_size, month_size, max_codes, lr, clipvalue):
...
print('############### Load and merge tabular & seq data ############################')
...
#visualise model
# model = lateFussion(vocab_size, month_size, max_codes, lr, clipvalue)
model = earlyFussion(vocab_size, month_size, max_codes, lr, clipvalue)
print(model.summary())
...
with tf.device('/GPU:0'):
    ...
    model.evaluate([Xt_val, Xs_val], y_val)
    model.evaluate([Xt_eval, Xs_eval], y_eval)
    model.evaluate([Xt_test, Xs_test], y_test)
    model.evaluate([Xt_testWales, Xs_testWales], y_testWales)
    model.evaluate([Xt_testScotland, Xs_testScotland], y_testScotland)
    
print("--- Evaluate time: %s seconds ---" % (time.time() - start_time))
    
model.save('../SeqModel/model_all_tabSeq.h5')
pickle.dump(history, open('../SeqModel/history_all_tabSeq.sav', 'wb'))
