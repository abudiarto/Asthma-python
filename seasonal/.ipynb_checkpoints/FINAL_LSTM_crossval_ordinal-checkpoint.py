...
for target_outcome in target_outcomes:
    ...
    for train_index, test_index in kf.split(X, y):
        ...
        fold+=1
        print(f'fold: {fold}')
        tab_train, tab_test = tab[train_index], tab[test_index]
        ...
        model = modelCombined()
        model.summary()
        # plot_model(model, to_file='../../MODELS/LSTM_simple_plot_ordinal.png', show_shapes=True, show_layer_names=True)

        earlyStopping = EarlyStopping(monitor='val_auc', patience=patience, verbose=1, mode='max', restore_best_weights=True)
        mcp_save = ModelCheckpoint('../../MODELS/lstm_ordinal_CV_'+str(fold)+'.keras', save_best_only=True, monitor='val_auc', mode='max') #not needed in CV
        ...
        history = model.fit([tab_train, X_train], y_train,
                            validation_split=.2,
                            # validation_data=([val_tab, val_X], val_y),
                            epochs=epochs, batch_size=batch_size, class_weight=class_weight, callbacks = [lr_scheduler, earlyStopping])

        pickle.dump(history, open('../../MODELS/history_lstm_ordinal_CV_'+str(fold)+'.sav', 'wb'))
            
        modelname = 'LSTM'
        summary_result_train.append((modelname, fold, target_outcome, event_rate, ) + summariseResult (tab_test, X_test, y_test, model))
        # with tf.device('/GPU:0'):
...
summary_result_train.to_csv('../../MODELS/LSTM_ordinal_CV.csv', index_label=False, index=False)
