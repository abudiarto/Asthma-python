...
for outcome in outcomes:
    ...
    for item in models_to_be_trained:
        ...
        pickle.dump(gs.cv_results_, open('../../MODELS/BSOrdinal/' + outcome.split('_')[-1] + '_' + item['model_name'] + '.sav', 'wb'))

print(pd.DataFrame(output, columns=['outcome', 'model', 'best_score', 'best_param']))
pd.DataFrame(output, columns=['outcome', 'model', 'best_score', 'best_param']).to_csv('../../MODELS/BS_result_ordinal.csv', index = False, index_label=False)
